...
```{r}
pm2_k[!(row.names(pm2_k) %in% c('88','92')), ] 
```
...
pm2_k <- pm2_k %>%
  mutate(
    res = map2(pm2_k, k_model, add_residuals)
  )
```
...
**vi) Lag et plot av residualene for Haugesund (jmf. slide 28 kap. 25).**
...
**vii. Bruk geom_smooth() til å vise utviklingen i gjennomsnittlige residualer for hvert fylke.**
...
**viii. La samme plot, men nå bare for fylkene 01, 02, 03, 07, 11 og 12.**

```{r}
pm2_k %>% filter(fnr %in% c("01", "02", "03", "07", "11", "12")) %>%
ggplot(mapping = aes(x = aar_d, y = resid)) +
scale_color_viridis(discrete = TRUE, option = "D") +
geom_line(aes(group = fnr, colour = fnr, size =fnr)) +
scale_size_manual(values = c(seq(2.0, 0.5, by = -0.1))) +
geom_hline(yintercept = 0) +
theme(legend.position = 'bottom')
```

Som over bare at du starter med filter(), husk %in%, for å plukke ut de
angitte kommunene. Bruk geom_line()
...
Tips: Først unnest(data) i en pipe, før du så gjør group_by og summarise
så piper du videre til ggplot()

**xi. Gjør det samme, men nå bare for fylkene “fnr_02”, “fnr_03”, “fnr_11” og “fnr_12”.**

-

**xii) Bruk funksjonen glance fra broom pakken til å lage en mod_summary variabel og unnest() denne. Legg resultatet i mod_sum (jmf. slide 43 kap. 25).**

-

**xiii) Bruk mod_sum til å lage et histogram av adj.r.squared for modellene våre.**
